...
#if LIBCRYPTO_VERSION >= 3
void sha_hash(const std::vector<uint8_t> &data, std::vector<uint8_t> &hash, const hash_type type)
{
  ...
  hash = std::vector<uint8_t>(EVP_MD_size(digest));
...
void hmac_hash(const std::vector<uint8_t> &data, std::vector<uint8_t> &hash, bool empty_key)
{
  ...
  {
#if LIBCRYPTO_VERSION >= 3
    sha_hash(data, key, hash_type::SHA1);
#else
    sha1_hash(data, key);
#endif
  }
  ...
  temp = (uint8_t *)::HMAC(EVP_sha1(), key.data(), key.size(), (unsigned char *)temp_data.data(),
                           temp_data.size(), NULL, NULL);
  hash = std::vector<uint8_t>(SHA_DIGEST_LENGTH);
...
void crc32_hash(const std::vector<uint8_t> &data, std::vector<uint8_t> &hash)
{
  ...
  std::uint_fast32_t temp =
      std::uint_fast32_t{0xFFFFFFFFuL} &
      ~std::accumulate(data.cbegin(), data.cend(),
                       ~std::uint_fast32_t{0} & std::uint_fast32_t{0xFFFFFFFFuL},
                       [](std::uint_fast32_t checksum, std::uint_fast32_t value)
